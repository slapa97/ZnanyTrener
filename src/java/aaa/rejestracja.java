...
public class rejestracja extends HttpServlet {
    ...
    String email = request.getParameter("email");
      ...
      ResultSet rs = sql.executeQuery("SELECT email,id_klient from klient where email='"+email+"';");


      if (rs.next()){
      if (0==email.compareTo(rs.getString(1))){

      response.sendRedirect("rejestracja22.jsp?email="+request.getParameter("email")+"");
polaczenie.close();
      ...
      response.sendRedirect("a_userdodaj4?email="+request.getParameter("email")+"");
polaczenie.close();
      }}
      else
      {
